...
def train():
    models = [RandomForestClassifier() for i in range(11)]
    ...
    for i in range(11):
        pickle.dump(models[i], open('./model/RF/model_for_transport_%d.pkl' % i, 'wb'))


def test(load=False):
    if load:
        pred = pickle.load(open('./test_pred_result.pkl', 'rb'))
        ...
        label = pickle.load(open('./test_data/label.pkl', 'rb'))
        for i in range(11):
            models.append(pickle.load(open('./model/RF/model_for_transport_%d.pkl' % i, 'rb')))
        pred = []
        right = 0
        count = 0
        for query in data:
        ...
        pickle.dump(pred, open('./model/RF/test_pred_result.pkl', 'wb'))
