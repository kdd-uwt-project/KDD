...
def data_gen(hour_m, train_index):
    ...
    for plan in train_plans:
        ...
        if index != 1:
            ...
            for eplan in plan[2]:
                if firstPlanTM == 0:
                    firstPlanTM = eplan['transport_mode']
                #index == 0
                data[index][eplan['transport_mode']-1].append([eindex, hour, eplan['distance'] / eplan['eta'], eplan['price'] / eplan['eta'] if eplan['price'] != '' else 0, eplan['eta'] / minETA, firstPlanTM])
                curLabel = 0
                if isClicked == 1 and target_mode == eplan['transport_mode']:
                    curLabel = 1
                elif isClicked == 0:
                    curLabel = 2
                data_label[index][eplan['transport_mode']-1].append(curLabel)
                eindex += 1
                if eindex > 3:
                    break
            count += 1
...
def test(hour_m, train_index):
    ...
    for query in data:
        ...
        for plan in query[2]:
            if firstPlanTM == 0:
                firstPlanTM = plan['transport_mode']
            hour = int(query[1].split(' ')[1].split(':')[0])
            hour = hour_m[hour]
            temp = [[pindex, hour, plan['distance'] / plan['eta'], plan['price'] / plan['eta'] if plan['price'] != '' else 0,
                     plan['eta'] / minETA, firstPlanTM]]
    ...
    output = open('./output/Xgboost/output'+ str(train_index) + '.txt', 'w')
...
def get_result(hour_m, train_index):
    ...
    models = []
    f = open("xgboost" + str(train_index) + ".csv", 'w')
    ...
    for query in data:
        ...
        for plan in query[2]:
            ...
            hour = int(query[1].split(' ')[1].split(':')[0])
            hour = hour_m[hour]
            temp = [[pindex, hour, plan['distance'] / minDis, plan['price'] / minPrice if plan['price'] != '' else 0, plan['eta'] / minETA, firstPlanTM]]
...
if __name__ == '__main__':
    ...
    data_gen(hour_m3, "_withFirstTM_isSelected_UnitPrice_unitDistance")
    train('_withFirstTM_isSelected_UnitPrice_unitDistance')
    test(hour_m3, '_withFirstTM_isSelected_UnitPrice_unitDistance')
    #get_result(hour_m3, '_withFirstTM_isSelected')
